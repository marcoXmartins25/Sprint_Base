...
function TaskRow({ task, delay }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl border border-gray-100 bg-white transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
    >
      <div
        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${task.status === "done" ? "bg-emerald-500 border-emerald-500" : task.status === "in-progress" ? "border-blue-400" : "border-gray-300"}`}
      >
        {task.status === "done" && (
          <svg
      ...
      <span
        className={`text-sm flex-1 ${task.status === "done" ? "line-through text-gray-400" : "text-gray-700"}`}
      >
        {task.title}
      </span>
      <span
      ...
      {task.hours > 0 && (
        <span className="text-xs text-gray-400">{task.hours}h</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200 shadow-xl shadow-gray-200/50 w-full max-w-lg">
      ...
      <div className="space-y-1.5">
        {TASKS.map((task, i) => (
          <TaskRow key={task.title} task={task} delay={i * 150} />
        ))}
      </div>
    </div>
  );
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <p className="text-lg text-gray-500 leading-relaxed mb-8 max-w-md">
                Plan sprints, track tasks with priorities and hours, assign team
                members, and export beautiful PDF reports — all in one place.
              </p>
            ...
            {[
              {
                ...
                desc: "Create sprints with dates, track progress with a live completion bar based on all task statuses.",
              },
              {
                icon: "✅",
                color: "bg-emerald-50",
                title: "Task Management",
                desc: "Priority levels, due dates, hour tracking, and inline editing — no modals needed.",
              },
              {
                icon: "👥",
                color: "bg-violet-50",
                title: "Team Assignment",
                desc: "Assign tasks to team members with avatar support. See who is working on what at a glance.",
              },
              {
                icon: "📊",
                color: "bg-blue-50",
                title: "Progress Tracking",
                desc: "Smart progress bar that weighs Done (100%), In Progress (50%) and To Do (0%).",
              },
              {
                icon: "📄",
                color: "bg-amber-50",
                title: "PDF Reports",
                desc: "Export beautiful, branded PDF reports with all task details, priorities and statuses.",
              },
              {
                icon: "🔒",
                color: "bg-rose-50",
                title: "Secure by Default",
                desc: "JWT authentication, protected file uploads, and per-user data isolation.",
              },
            ].map(({ icon, color, title, desc }) => (
              <div
                key={title}
                className="bg-white rounded-2xl p-6 border border-gray-100 hover:shadow-lg hover:shadow-gray-100 hover:-translate-y-1 transition-all duration-300 group"
              >
                <div
                ...
                <h3 className="text-base font-semibold text-gray-900 mb-2 group-hover:text-indigo-600 transition">
                  {title}
                </h3>
              ...
              {[
                {
                  ...
                  desc: "Set a title and pick your start/end dates. The sprint card appears instantly on your dashboard.",
                },
                {
                  n: "2",
                  title: "Add Tasks",
                  desc: 'Click "+ Add Task" and fill in title, priority, dates, hours and assignee. Edit anything inline directly in the table.',
                },
                {
                  n: "3",
                  title: "Track & Update",
                  desc: "Click on Priority or Status to change them with a dropdown. Watch the progress bar update in real time.",
                },
                {
                  n: "4",
                  title: "Export PDF",
                  desc: 'Hit "↓ PDF" to download a branded report with all task details, ready to share with stakeholders.',
                },
              ].map(({ n, title, desc }) => (
      ...
      <footer className="py-8 px-6 border-t border-gray-100">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <p className="text-xs text-gray-400">
            Ship faster. Stay organized.<br></br> V 0.0.1
          </p>
        </div>
      </footer>
    </div>
